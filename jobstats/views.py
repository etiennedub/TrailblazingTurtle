...
from userportal.common import (
    user_or_staff,
    username_to_uid,
    Prometheus,
    request_to_username,
    compute_allocations_by_user,
    get_step,
    parse_start_end,
    fixed_zoom_config,
)
...
def context_job_info(request, username, job_id):
    ...
    context["uid"] = uid
    jobs = (
        JobTable.objects.filter(id_user=uid)
        .filter(id_job__in=jobid_str_to_list(job_id))
        .all()
    )
    if len(jobs) == 0:
        raise Http404()
...
def display_compute_name(lines, line):
    ...
    names = [
        entry["metric"][settings.PROM_NODE_HOSTNAME_LABEL].split(":")[0]
        for entry in lines
    ]
    if len(set(names)) == 1:
...
def user(request, username):
    ...
    try:
        query_cpu = 'sum(rate(slurm_job_core_usage_total{{user="{}", {}}}[{}s]) / 1000000000)'.format(
            username, prom.get_filter(), prom.rate("slurm-job-exporter")
        )
        stats_cpu = prom.query_prometheus(query_cpu, now - delta, now)
        context["cpu_used"] = statistics.mean(stats_cpu[1])
        context["cpu_used_p90"] = statistics.quantiles(
            stats_cpu[1], n=10, method="inclusive"
        )[-1]
    except ValueError:
    ...
    try:
        query_mem = 'sum(slurm_job_memory_max{{user="{}", {}}})'.format(
            username, prom.get_filter()
        )
    ...
    try:
        query_gpu = 'sum(slurm_job_utilization_gpu{{user="{}", {}}})/100'.format(
            username, prom.get_filter()
        )
    ...
    if request.user.is_staff:
        context["notes"] = (
            Note.objects.filter(username=username)
...
def job(request, username, job_id):
    ...
    if job.id_array_job != 0:
        ...
        context["array_jobs"] = JobTable.objects.filter(
            id_array_job=job.id_array_job
        ).order_by("id_array_task")

    if request.user.is_staff:
        context["notes"] = (
            Note.objects.filter(job_id=job_id)
            .filter(deleted_at=None)
            .order_by("-modified_at")
        )
    ...
    comments = []
    if (
        "--dependency=singleton" in job.submit_line
        or "--depend=singleton" in job.submit_line
        or "-d singleton" in job.submit_line
    ):
        comments += [Comment(_("This job is using a singleton dependency"), "info")]
    if len(context["dependencies"]) > 0:
        comments += [Comment(_("This job has dependencies on other jobs"), "info")]
    if len(context["depends_on_this"]) > 0:
        comments += [Comment(_("This job is a dependency for other jobs"), "info")]
    if "--exclusive" in job.submit_line:
        comments += [Comment(_("This job is using exclusive mode"), "info")]
    if "--licenses=" in job.submit_line or "-L " in job.submit_line:
        comments += [Comment(_("This job is using licenses"), "info")]
    if "--nodelist=" in job.submit_line or "-w " in job.submit_line:
        comments += [Comment(_("This job is using a specific nodelist"), "info")]
    if "--exclude=" in job.submit_line or "-x " in job.submit_line:
        comments += [Comment(_("This job is excluding nodes"), "warning")]
    if "--requeue" in job.submit_line:
        comments += [Comment(_("This job can be requeued"), "info")]
    if "--no-requeue" in job.submit_line:
        comments += [Comment(_("This job cannot be requeued"), "info")]
    if "--reservation=" in job.submit_line:
        comments += [Comment(_("This job is using a reservation"), "info")]
    if "--switches=" in job.submit_line:
        comments += [
            Comment(_("This job is using a maximum quantity of switches"), "info")
        ]

    if "slurm_exporter" in settings.EXPORTER_INSTALLED:
        try:
            query_priority = (
                'slurm_account_levelfs{{account="{account}", {filter}}}'.format(
                    account=job.account, filter=prom.get_filter()
                )
            )
            if job.time_start_dt() is not None:
                # If the job has started, use the start time.
                stats_priority = prom.query_prometheus(
                    query_priority,
                    job.time_start_dt(),
                    job.time_start_dt() + timedelta(minutes=15),
                )
    ...
    try:
        query_cpu = 'sum(rate(slurm_job_core_usage_total{{slurmjobid="{}", {}}}[{}s]) / 1000000000)'.format(
            job_id, prom.get_filter(), prom.rate("slurm-job-exporter")
        )
        stats_cpu = prom.query_prometheus(
            query_cpu, job.time_start_dt(), job.time_end_dt()
        )
        context["cpu_used"] = statistics.mean(stats_cpu[1])
        context["cpu_used_p90"] = statistics.quantiles(
            stats_cpu[1], n=10, method="inclusive"
    ...
    try:
        query_cpu_bynode = (
            'count(slurm_job_core_usage_total{{slurmjobid="{}", {}}}) by ({})'.format(
                job_id, prom.get_filter(), settings.PROM_NODE_HOSTNAME_LABEL
            )
        )
        stats_cpu_bynode = prom.query_prometheus_multiple(
            query_cpu_bynode, job.time_start_dt(), job.time_end_dt()
        )
        cpu_bynode = []
    ...
    try:
        query_mem = 'sum(slurm_job_memory_max{{slurmjobid="{}", {}}})'.format(
            job_id, prom.get_filter()
        )
        stats_mem = prom.query_prometheus(
            query_mem, job.time_start_dt(), job.time_end_dt()
        )
        context["mem_used"] = max(stats_mem[1])
    except ValueError:
    ...
    if context["gpu_count"] > 0:
        try:
            query_gpu_util = (
                'sum(slurm_job_utilization_gpu{{slurmjobid="{}", {}}})'.format(
                    job_id, prom.get_filter()
                )
            )
            stats_gpu_util = prom.query_prometheus(
                query_gpu_util, job.time_start_dt(), job.time_end_dt()
            )
        ...
        try:
            query_gpu_mem = 'sum(slurm_job_memory_usage_gpu{{slurmjobid="{}", {}}})/(1024*1024*1024)'.format(
                job_id, prom.get_filter()
            )
            stats_gpu_mem = prom.query_prometheus(
                query_gpu_mem, job.time_start_dt(), job.time_end_dt()
            )
            context["gpu_mem"] = (
                max(stats_gpu_mem[1]) / GPU_MEMORY[job.gpu_type()] * 100
            )
        except ValueError:
        ...
        try:
            query_gpu_power = (
                'sum(slurm_job_power_gpu{{slurmjobid="{}", {}}})/(1000)'.format(
                    job_id, prom.get_filter()
                )
            )
            stats_gpu_power = prom.query_prometheus(
                query_gpu_power, job.time_start_dt(), job.time_end_dt()
            )
            used_power = (
                statistics.mean(stats_gpu_power[1]) - GPU_IDLE_POWER[job.gpu_type()]
            )
    ...
    try:
        ...
        try:
            ...
            comments += analyze_jobscript(
                context["job_script"].submit_script, context["loaded_modules"], job
            )
        except ValueError:
            context["loaded_modules"] = (
                None  # Could not parse jobscript to find loaded modules
            )
    ...
    if context["cpu_used"] is not None:
        if context["cpu_used"] < 1 and context["tres_req"]["total_cores"] > 1:
            comments += [
                Comment(
                    _(
                        "Less than 1 core was used on average but {} were asked for, this look like a serial job"
                    ).format(context["tres_req"]["total_cores"]),
                    "critical",
        ...
        if (context["tres_req"]["total_cores"] / 2) > context["cpu_used"] and (
            ...
            comments += [
                Comment(
                    _("Less than half the CPU compute cycle were used").format(
                        context["tres_req"]["total_cores"]
                    ),
                    "critical",
                    graph_ids=["cpu"],
                )
    ...
    if job.state == JobTable.StatesJob.COMPLETE:
        if job.timelimit < 60:  # in minutes
            comments += [
                Comment(
                    _(
                        "Less than 1 hour was asked, please packages your short jobs with GLOST or GNU parallel if you want to run 100+ short jobs"
                    ),
                    "critical",
            ...
            comments += [
                Comment(
                    _(
                        "Less than 1 hour was used, please packages your short jobs with GLOST or GNU parallel if you want to run 100+ short jobs"
                    ),
                    "warning",
                    "https://docs.alliancecan.ca/wiki/GLOST",
                )
    ...
    if job.state == JobTable.StatesJob.NODE_FAIL:
        comments += [
            Comment(
                _(
                    "Node failure, this is a temporary issue and probably not caused by the job"
                ),
                "critical",
            )
        ]
    ...
    if len(job.nodes()) > 1:
        comments += [Comment(_("This job is using multiple nodes"), "info")]

    try:
        query_threads = 'sum(slurm_job_threads_count{{slurmjobid=~"{}", state="running", {}}})'.format(
            job_id, prom.get_filter()
        )
        stats_threads = prom.query_prometheus(
            query_threads, job.time_start_dt(), job.time_end_dt()
        )
        ...
        if running_threads > 1.25 * context["tres_req"]["total_cores"]:
            comments += [
                Comment(
                    _(
                        "This job is running on average {:.1f} threads on {} cores, the cores might be oversubscribed"
                    ).format(running_threads, context["tres_req"]["total_cores"]),
                    "warning",
                    graph_ids=["thread"],
                )
            ]
            ...
            comments += [
                Comment(
                    _(
                        "This job is running on average {:.1f} threads on {} cores, the cores might be underused"
                    ).format(running_threads, context["tres_req"]["total_cores"]),
                    "warning",
                    graph_ids=["thread"],
                )
    ...
    try:
        query_exe = 'sum(deriv(slurm_job_process_usage_total{{slurmjobid=~"{}", {}}}[1m])) by (exe)'.format(
            job_id, prom.get_filter()
        )
        stats_exe = prom.query_prometheus_multiple(
            query_exe, job.time_start_dt(), job.time_end_dt()
        )
    ...
    try:
        ...
        started = (
            EventTable.objects.filter(node_name__in=job.nodes())
            .filter(time_start__gte=start)
            .filter(time_start__lte=end)
            .all()
        )
...
def graph_cpu(request, username, job_id):
    ...
    query = 'rate(slurm_job_core_usage_total{{slurmjobid=~"{}", {}}}[{}s]) / 1000000000'.format(
        context["id_regex"], prom.get_filter(), prom.rate("slurm-job-exporter")
    )
    ...
    for line in stats:
        ...
        if context["multiple_jobs"]:
            name = "{} Core {} {}".format(
                line["metric"]["slurmjobid"], core_num, compute_name
            )
        else:
    ...
    if context["multiple_jobs"]:
        query_count = (
            'count(slurm_job_core_usage_total{{slurmjobid=~"{}", {}}})'.format(
                context["id_regex"], prom.get_filter()
            )
        )
...
def graph_cpu_user(request, username):
    ...
    try:
        query_used = (
            'sum(slurm_job:used_core:sum_user_account{{user="{}", {}}})'.format(
                username, prom.get_filter()
            )
        )
        stats_used = prom.query_prometheus(
            query_used, request.start, request.end, step=request.step
        )
        data.append(
            {
                "x": list(
                    map(lambda x: x.strftime("%Y-%m-%d %H:%M:%S"), stats_used[0])
                ),
                "y": stats_used[1],
                "type": "scatter",
                "name": _("Used"),
        ...
        query_alloc = (
            'sum(slurm_job:allocated_core:count_user_account{{user="{}", {}}})'.format(
                username, prom.get_filter()
            )
        )
        stats_alloc = prom.query_prometheus(
            query_alloc, request.start, request.end, step=request.step
        )
        data.append(
            {
                "x": list(
                    map(lambda x: x.strftime("%Y-%m-%d %H:%M:%S"), stats_alloc[0])
                ),
                "y": stats_alloc[1],
                "type": "scatter",
                "name": _("Allocated"),
...
def graph_mem_user(request, username):
    data = []
    try:
        query_alloc = 'sum(slurm_job:allocated_memory:sum_user_account{{user="{}", {}}})/(1024*1024*1024)'.format(
            username, prom.get_filter()
        )
        stats_alloc = prom.query_prometheus(
            query_alloc, request.start, request.end, step=request.step
        ...
        query_max = 'sum(slurm_job:max_memory:sum_user_account{{user="{}", {}}})/(1024*1024*1024)'.format(
            username, prom.get_filter()
        )
        stats_max = prom.query_prometheus(
            query_max, request.start, request.end, step=request.step
        )
        data.append(
            {
        ...
        query_used = 'sum(slurm_job:rss_memory:sum_user_account{{user="{}", {}}})/(1024*1024*1024)'.format(
            username, prom.get_filter()
        )
        stats_used = prom.query_prometheus(
            query_used, request.start, request.end, step=request.step
...
def graph_mem(request, username, job_id):
    ...
    for stat in stat_types:
        query = '{}{{slurmjobid=~"{}", {}}}/(1024*1024*1024)'.format(
            stat[0], context["id_regex"], prom.get_filter()
        )
        stats = prom.query_prometheus_multiple(
            query,
            context["start"],
        ...
        for line in stats:
            ...
            if context["multiple_jobs"]:
                name = "{} {} {}".format(
                    line["metric"]["slurmjobid"], stat[1], compute_name
                )
            else:
    ...
    if context["multiple_jobs"]:
        query_count = 'sum(slurm_job_memory_limit{{slurmjobid=~"{}", {}}})/(1024*1024*1024)'.format(
            context["id_regex"], prom.get_filter()
        )
        line = prom.query_prometheus(
            query_count,
...
def graph_thread(request, username, job_id):
    ...
    query_procs = 'slurm_job_process_count{{slurmjobid=~"{}", {}}}'.format(
        context["id_regex"], prom.get_filter()
    )
    ...
    for line in stats_procs:
        ...
        if context["multiple_jobs"]:
            name = "{} {} {}".format(
                line["metric"]["slurmjobid"], _("Processes"), compute_name
            )
        else:
    ...
    query_threads = 'slurm_job_threads_count{{slurmjobid=~"{}", {}}}'.format(
        context["id_regex"], prom.get_filter()
    )
...
def graph_lustre_mdt(request, username, job_id):
    ...
    for line in stats:
        ...
        if context["multiple_jobs"]:
            name = "{} {} {}".format(
                line["metric"]["jobid"], operation.capitalize(), fs
            )
        else:
...
def graph_lustre_mdt_user(request, username):
    query = 'sum(rate(lustre_job_stats_total{{component=~"mdt",user=~"{}", {}}}[{}s])) by (operation, fs) !=0'.format(
        username, prom.get_filter(), prom.rate("lustre_exporter")
    )
    stats = prom.query_prometheus_multiple(
        query, request.start, request.end, step=request.step
    )
    data = []
    for line in stats:
        operation = line["metric"]["operation"]
...
def graph_lustre_ost_user(request, username):
    ...
    for i in ["read", "write"]:
        query = '(sum(rate(lustre_job_{}_bytes_total{{component=~"ost",user=~"{}",target=~".*-OST.*", {}}}[{}s])) by (fs)) / (1024*1024)'.format(
            i, username, prom.get_filter(), prom.rate("lustre_exporter")
        )
        stats = prom.query_prometheus_multiple(
            query, request.start, request.end, step=request.step
        )
...
def graph_gpu_utilization(request, username, job_id):
    ...
    for q in queries:
        query = '{}{{slurmjobid=~"{}", {}}}'.format(
            q[0], context["id_regex"], prom.get_filter()
        )
        stats = prom.query_prometheus_multiple(
            query,
            context["start"],
        ...
        for line in stats:
            ...
            if context["multiple_jobs"]:
                name = "{} {} GPU {} {}".format(
                    line["metric"]["slurmjobid"], q[1], gpu_id, compute_name
                )
            else:
...
def graph_gpu_utilization_user(request, username):
    ...
    query_alloc = (
        'sum(slurm_job:allocated_gpu:count_user_account{{user="{}", {}}})'.format(
            username, prom.get_filter()
        )
    )
    stats_alloc = prom.query_prometheus(
    ...
    query_used = 'sum(slurm_job:used_gpu:sum_user_account{{user="{}", {}}})'.format(
        username, prom.get_filter()
    )
    stats_used = prom.query_prometheus(
        query_used, request.start, request.end, step=request.step
...
def graph_gpu_memory_utilization(request, username, job_id):
    ...
    query = 'slurm_job_utilization_gpu_memory{{slurmjobid=~"{}", {}}}'.format(
        context["id_regex"], prom.get_filter()
    )
    stats = prom.query_prometheus_multiple(
        query,
    ...
    for line in stats:
        ...
        if context["multiple_jobs"]:
            name = "{} GPU {} {}".format(
                line["metric"]["slurmjobid"], gpu_id, compute_name
            )
        else:
...
def graph_gpu_memory(request, username, job_id):
    ...
    query = (
        'slurm_job_memory_usage_gpu{{slurmjobid=~"{}", {}}} /(1024*1024*1024)'.format(
            context["id_regex"], prom.get_filter()
        )
    )
    stats = prom.query_prometheus_multiple(
...
def graph_gpu_power(request, username, job_id):
    ...
    query = 'slurm_job_power_gpu{{slurmjobid=~"{}", {}}}/1000'.format(
        context["id_regex"], prom.get_filter()
    )
    stats = prom.query_prometheus_multiple(
        query,
...
def graph_gpu_power_user(request, username):
    ...
    query_alloc = 'count(slurm_job_power_gpu{{user="{}", {}}}) by (gpu_type)'.format(
        username, prom.get_filter()
    )
    ...
    query_used = (
        'sum(slurm_job_power_gpu{{user="{}", {}}}) by (gpu_type) / 1000'.format(
            username, prom.get_filter()
        )
    )
...
def graph_gpu_pcie(request, username, job_id):
    ...
    for line in stats:
        ...
        if context["multiple_jobs"]:
            name = "{} GPU {} {} {}".format(
                line["metric"]["slurmjobid"], direction, gpu_id, compute_name
            )
        else:
...
def graph_ethernet_bdw(request, username, job_id):
    ...
    for direction in ["receive", "transmit"]:
        ...
        stats = prom.query_prometheus_multiple(
            query, context["start"], context["end"], step=step
        )
        for line in stats:
            compute_name = display_compute_name(stats, line)
            ...
            data.append(
                {
                    "x": list(
                        map(lambda x: x.strftime("%Y-%m-%d %H:%M:%S"), line["x"])
                    ),
                    "y": y,
                    "type": "scatter",
...
def graph_disk_iops(request, username, job_id):
    ...
    for direction in ["reads", "writes"]:
        ...
        for line in stats:
            compute_name = "{} {}".format(
                line["metric"]["device"], display_compute_name(stats, line)
            )
            y = line["y"]
            if direction == "reads":
                y = line["y"]
...
def graph_disk_used(request, username, job_id):
    ...
    for line in stats_disk:
        compute_name = "{} {}".format(
            line["metric"]["device"], display_compute_name(stats_disk, line)
        )
        x = list(map(lambda x: x.strftime("%Y-%m-%d %H:%M:%S"), line["x"]))
        y = line["y"]
        data.append(
    ...
    for direction in ["Reads", "Writes"]:
        ...
        for line in stats:
            ...
            compute_name = "{} socket {} {}".format(
                direction, line["metric"]["socket"], display_compute_name(stats, line)
            )
            x = list(map(lambda x: x.strftime("%Y-%m-%d %H:%M:%S"), line["x"]))
            y = line["y"]
            data.append(
...
def map_pcm_cores(request, context):
    ...
    for line in stats_cpu:
        ...
        if instance not in used_mapping:
            used_mapping[instance] = (
                set()
            )  # the set of physical cores/sockets used by the job
        used_mapping[instance].add(
            all_cores_mapping[instance][int(line["metric"]["core"])]
        )
...
def graph_cpu_interconnect(request, username, job_id):
    ...
    for line in stats:
        compute_name = "Received socket {} {}".format(
            line["metric"]["socket"], display_compute_name(stats, line)
        )
        x = list(map(lambda x: x.strftime("%Y-%m-%d %H:%M:%S"), line["x"]))
        y = line["y"]
        data.append(
...
+ ( label_replace(sum(slurm_job_power_gpu{{slurmjobid="{jobid}", {filter}}} / 1000) by ({hostname_label}),"{hostname_label}", "$1", "{hostname_label}", "(.*):.*") )'.format(
    ...
    return prom.query_prometheus_multiple(
        query, job.time_start_dt(), job.time_end_dt(), step
    )
...
def graph_power(request, username, job_id):
    ...
    data = []
    for line in power(
        context["job"], step=max(context["step"], prom.rate("redfish_exporter"))
    ):
...
def value_cost(request, username, job_id):
    ...
    if "redfish_exporter" in settings.EXPORTER_INSTALLED:
        ...
        for line in power(job, step=prom.rate("redfish_exporter")):
            ...
            kw = (
                sum(line["y"]) / len(line["y"])
            ) / 1000  # compute the average power consumption
            kwhs.append(kw * hours)
        kwh = sum(kwhs)
        response["kwh"] = kwh
        if settings.ELECTRIC_CAR_RANGE_KM_PER_KWH:
            response["electric_car_range_km"] = kwh * float(
                settings.ELECTRIC_CAR_RANGE_KM_PER_KWH
            )
        if settings.ELECTRICITY_COST_PER_KWH:
            response["electricity_cost_dollar"] = kwh * float(
                settings.ELECTRICITY_COST_PER_KWH
            )
    ...
    if job.gpu_count() > 0:
        ...
        if settings.GPU_COST_PER_HOUR:
            response["hardware_cost_dollar"] = (
                job.gpu_count() * settings.GPU_COST_PER_HOUR * hours
            )

        if settings.CLOUD_GPU_COST_PER_HOUR:
            response["cloud_cost_dollar"] = (
                job.gpu_count() * settings.CLOUD_GPU_COST_PER_HOUR * hours
            )
    else:
        # Cost for a CPU job
        if settings.CPU_CORE_COST_PER_HOUR:
            response["hardware_cost_dollar"] = (
                job.parse_tres_req()["total_cores"]
                * settings.CPU_CORE_COST_PER_HOUR
                * hours
            )
        if settings.CLOUD_CPU_CORE_COST_PER_HOUR:
            response["cloud_cost_dollar"] = (
                job.parse_tres_req()["total_cores"]
                * settings.CLOUD_CPU_CORE_COST_PER_HOUR
                * hours
            )
